    ...
    methods: {
        changePage(page) {
            ...
            if (page == "Home") {
                ...
                $.post('http://vrp_mdt/getWarrants', JSON.stringify({}));
        ...
        closeMDT() {
            $.post('http://vrp_mdt/close', JSON.stringify({}));
        },
        OffenderSearch() {
            if (this.offender_search) {

                this.offender_results.query = this.offender_search;
                $.post('http://vrp_mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://vrp_mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://vrp_mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://vrp_mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://vrp_mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://vrp_mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        DeleteSelectedReport() {
            $.post('http://vrp_mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://vrp_mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://vrp_mdt/vehicleSearch', JSON.stringify({
        ...
        OpenVehicleDetails(result) {
            $.post('http://vrp_mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://vrp_mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://vrp_mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://vrp_mdt/getReport', JSON.stringify({
                id: id
            }));
            return;
        },
    ...
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                console.log(JSON.stringify(mdtApp.offender_results.results));
                ...
                mdtApp.offender_search = mdtApp.offender_selected.firstname + " " + mdtApp.offender_selected.name;
